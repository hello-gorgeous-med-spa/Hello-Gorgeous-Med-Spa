...
function BulletList({ items }: { items: string[] }) {
  if (items.length === 0) {
    return <p className="text-sm text-white/45 italic">Not emphasized on this view — ask your provider.</p>;
  }
  return (
    <ul className="space-y-2.5 text-sm text-white/90 leading-relaxed list-disc pl-4 marker:text-[#FF2D8E]">
      {items.map((line) => (
        <li key={line}>{line}</li>
      ))}
    </ul>
    ...
    <>
      ...
      <Section className="relative">
        ...
        <div className="relative z-10">
          <FadeUp>
            <p className="text-[#FF2D8E] text-lg md:text-xl font-medium mb-6 tracking-wide">RX & COMPOUNDING</p>
            ...
            <ul className="mt-8 space-y-2 text-white/80 text-sm md:text-base max-w-3xl list-disc pl-5 marker:text-[#FF2D8E]">
            ...
            <div className="mt-10 flex flex-col sm:flex-row gap-4">
              ...
              <Link
                ...
                className="inline-flex items-center justify-center min-h-[48px] px-8 py-4 rounded-full border-2 border-white/30 text-white font-semibold hover:border-[#FF2D8E] hover:text-[#FF2D8E] transition"
              >
      ...
      <Section className="bg-black border-t border-white/10">
        <div className="space-y-16 md:space-y-20">
          <FadeUp>
            <h2 className="text-2xl md:text-3xl font-bold text-white mb-2">By category</h2>
            <p className="text-white/70 max-w-2xl text-sm md:text-base">
              <span className="font-semibold text-white/90">Take-home</span> usually means capsules, creams, troches,
              nasal sprays, or home injections after you are trained.{" "}
              <span className="font-semibold text-white/90">In-clinic</span> usually means IV/IM programs, draws,
          ...
          {PRODUCT_CATEGORIES.map((cat) => (
            <FadeUp key={cat.id}>
              <article className="rounded-2xl border border-white/10 bg-white/[0.03] p-6 md:p-8">
                ...
                {cat.blurb ? <p className="mt-2 text-sm text-white/65 max-w-3xl">{cat.blurb}</p> : null}
                <div className="mt-6 grid gap-8 md:grid-cols-2 md:gap-10">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-wider text-[#FF2D8E] mb-3">
                      Take-home (typical)
                    </p>
                    <BulletList items={cat.takeHome} />
                  </div>
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-wider text-rose-300/90 mb-3">
      ...
      <Section className="bg-gradient-to-b from-black to-zinc-950 border-t border-white/10">
        <FadeUp>
          <p className="text-white/75 text-sm md:text-base leading-relaxed max-w-3xl">{PRODUCTS_OFFER_FOOTNOTE}</p>
