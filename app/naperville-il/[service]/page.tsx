...
export function generateStaticParams() {
  return napervilleServiceSlugs.map((service) => ({ service }));
}
...
export function generateMetadata({
  ...
  if (!s) {
    return pageMetadata({
      title: "Naperville, IL Service",
      description: "Service details for Naperville, IL.",
      path: "/naperville-il",
    });
  }

  return pageMetadata({
    title: `${s.name} Near Naperville, IL`,
    description: `${s.name} near Naperville, IL — ${s.short} Book a consultation at Hello Gorgeous Med Spa.`,
    path: `/naperville-il/${s.slug}`,
  });
}

function napervilleFaqs(serviceName: string) {
  return [
    {
      question: `Do you offer ${serviceName} for Naperville clients?`,
      answer:
        "Yes—Hello Gorgeous Med Spa serves clients from Naperville. Our clinic is located in Oswego, IL.",
    },
    {
...
export default function NapervilleServicePage({ params }: { params: { service: string } }) {
  if (!napervilleServiceSlugs.includes(params.service as (typeof napervilleServiceSlugs)[number])) {
    notFound();
  }
    ...
    <>
        ...
        <div className="relative z-10">
          <FadeUp>
            ...
            <div className="mt-10 flex flex-col sm:flex-row gap-4">
              ...
              <CTA href="/naperville-il" variant="outline">
                Back to Naperville hub
              </CTA>
            </div>
          ...
          <div className="lg:col-span-7">
            <FadeUp>
              ...
              <p className="mt-4 text-black max-w-2xl">
                This page helps Naperville clients understand next steps. For full details and FAQs,
            ...
            <div className="mt-10 grid gap-4">
              {[
                ...
                {
                  t: "Serving Naperville",
                  b: "Convenient for Naperville clients scheduling in Oswego.",
                },
              ].map((x, idx) => (
      ...
      <Section>
        <FadeUp>
          ...
          <p className="mt-4 text-black max-w-2xl">
            Quick answers for local patients searching for {s.name} near Naperville, IL.
          </p>
        </FadeUp>

        <div className="mt-10 grid gap-4">
          {napervilleFaqs(s.name).map((f, idx) => (
