...
export default function AdminWellnessPage() {
  ...
  return (
    <div className="p-6 space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Wellness Members</h1>
        <p className="text-gray-600">Precision Hormone & Metabolic Reset programs</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          ...
          <p className="text-sm text-gray-600">Lab uploads</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          ...
          <div className="divide-y divide-gray-100">
            {refillRequests.map((r: any) => (
              <div key={r.id} className="px-6 py-4 flex items-center justify-between">
                <div>
                  <p className="font-medium text-gray-900">{r.med_name}</p>
          ...
          <div className="divide-y divide-gray-100 max-h-64 overflow-y-auto">
            {recentMessages.map((m: any) => (
              <div key={m.id} className="px-6 py-3">
                <p className="text-sm text-gray-900 line-clamp-2">{m.message_body}</p>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  <th className="text-left px-6 py-3 font-medium text-gray-700">Client</th>
                  <th className="text-left px-6 py-3 font-medium text-gray-700">Program</th>
                  <th className="text-left px-6 py-3 font-medium text-gray-700">Status</th>
                  <th className="text-left px-6 py-3 font-medium text-gray-700">Credits</th>
                </tr>
              </thead>
              <tbody>
