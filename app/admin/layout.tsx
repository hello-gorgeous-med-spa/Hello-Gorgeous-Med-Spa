// ============================================================
// ADMIN LAYOUT — Med Spa Operating System
// PRD: 12-item main nav, Owner portal, role-based visibility
// ============================================================
...
// Single flat nav: PRD main sidebar (12 items)
...
// Role-based nav: which hrefs each role can see (middleware still enforces route access)
function visibleHrefs(role: AdminRole): Set<string> {
  if (!role) return new Set(MAIN_NAV.map((n) => n.href)); // default: show all until session loads
  ...
  switch (role) {
    ...
    case 'staff':
      ...
      return all;
    case 'provider':
      // Provider: Charting + Services + Settings; no Memberships, Inventory, Marketing, Staff, Reports, Website
    ...
    <ToastProvider>
      <KeyboardShortcutsProvider>
        <div className="min-h-screen bg-[#F3F7F8] admin-panel">
          ...
          <div className="flex">
            {/* Sidebar — 12-item main nav + Owner (role-based) + POS */}
            <aside className="w-56 bg-white min-h-[calc(100vh-56px)] sticky top-14 hidden lg:flex lg:flex-col overflow-y-auto shadow-sm border-r border-black/10">
              <div className="p-3 border-b border-black/10">
                ...
                <Link
                  href="/pos"
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all text-sm ${showOwnerLink(role) ? 'mt-0.5' : ''} ${
              ...
              <div className="p-3 border-t border-black/10">
                <Link
                  ...
                  className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-black hover:bg-black/5"
                >
            ...
            {/* Mobile bottom nav — Dashboard, Calendar, Clients, POS */}
