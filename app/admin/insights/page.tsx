    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="flex gap-2 border-b border-black">
        <button
          ...
          className={`px-4 py-2 text-sm font-medium rounded-t-lg transition-colors ${tab === 'insights' ? 'bg-white border border-b-0 border-black text-violet-600 -mb-px' : 'text-black hover:bg-white'}`}
        >
          Insights
        </button>
        <RoleGate roles={['owner']}>
          <button
            ...
            className={`px-4 py-2 text-sm font-medium rounded-t-lg transition-colors ${tab === 'commands' ? 'bg-white border border-b-0 border-black text-violet-600 -mb-px' : 'text-black hover:bg-white'}`}
          >
      ...
      <div className="bg-white rounded-2xl border border-black shadow-sm overflow-hidden">
        ...
        <div className="h-[400px] overflow-y-auto p-6 space-y-4 bg-white0">
          {messages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center">
              ...
              <h3 className="text-lg font-semibold text-black mb-2">Ask me anything!</h3>
              <p className="text-black text-sm max-w-md mb-6">
              ...
              <div className="flex flex-wrap gap-2 justify-center max-w-lg">
                {QUICK_QUESTIONS.slice(0, 4).map((q, i) => (
                  <button
                    ...
                    className="px-3 py-1.5 bg-white border border-black rounded-full text-sm text-black hover:bg-white hover:border-black transition-colors"
                  >
                    {q}
                  </button>
                ))}
            ...
            <>
              ...
              {loading && (
                <div className="flex justify-start">
                  <div className="bg-white border border-black rounded-2xl px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-black">Analyzing...</span>
                    </div>
                  </div>
                </div>
              )}
        ...
        {/* Input Area */}
        <form onSubmit={handleSubmit} className="p-4 border-t border-black bg-white">
          <div className="flex items-center gap-3">
            <div className="flex-1 relative">
              <input
                ...
                className="w-full px-4 py-3 pr-12 bg-white border border-black rounded-xl focus:ring-2 focus:ring-violet-500 focus:border-violet-500 transition-colors"
                disabled={loading}
              />
              <button
                type="submit"
            ...
            <div className="flex flex-wrap gap-2 mt-3">
              {QUICK_QUESTIONS.slice(4).map((q, i) => (
                <button
                  ...
                  type="button"
                  onClick={() => handleQuickQuestion(q)}
                  className="px-3 py-1 bg-white rounded-full text-xs text-black hover:bg-white transition-colors"
                >
                  {q}
                </button>
              ))}
      ...
      <RoleGate roles={['owner']}>
        <div className="flex gap-2 mb-3">
          <button type="button" onClick={() => setCommandsView('chat')} className={`px-3 py-1.5 text-sm font-medium rounded-lg ${commandsView === 'chat' ? 'bg-amber-500 text-white' : 'bg-white text-black hover:bg-white'}`}>Chat</button>
          ...
          <div className="h-[420px] overflow-y-auto p-6 space-y-4 bg-white0">
            {commandsView === 'activity' ? (
              loadingActivity ? (
                ...
                <p className="text-black text-sm">No admin command activity yet.</p>
              ) : (
                <ul className="space-y-3">
                  {activityLogs.map((log) => {
                    ...
                    return (
                      <li key={log.id} className="flex items-start justify-between gap-3 p-3 bg-white border border-black rounded-lg">
                        <div className="min-w-0 flex-1">
                          <p className="text-xs text-black">{when}</p>
                          <p className="text-sm font-medium text-black mt-0.5">{approved ? 'Executed' : 'Proposed'}</p>
                          <p className="text-xs text-black mt-1">{location}</p>
                          <p className="text-xs text-black mt-0.5">{oldVal} → {newVal}</p>
                        </div>
                        {isLastExecution && (
              ...
              <div className="h-full flex flex-col items-center justify-center text-center">
                ...
                <h3 className="text-lg font-semibold text-black mb-2">Admin Commands</h3>
                <p className="text-black text-sm max-w-md mb-6">
              ...
              <>
                {cmdMessages.map((msg) => (
                    <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[85%] rounded-2xl px-4 py-3 ${msg.role === 'user' ? 'bg-violet-600 text-white' : 'bg-white border border-black text-black'}`}>
                        <p className="text-sm whitespace-pre-wrap">{msg.content}</p>
                        {msg.proposal && (
                          <div className="mt-3 pt-3 border-t border-black text-xs text-black">
                        ...
                        {msg.draft && (
                          <div className="mt-3 pt-3 border-t border-black">
                            <pre className="text-xs bg-white p-3 rounded-lg whitespace-pre-wrap font-sans overflow-x-auto">{msg.draft.draftText}</pre>
                            <p className="text-xs text-black mt-2">{msg.draft.note}</p>
                            <button type="button" onClick={() => { navigator.clipboard.writeText(msg.draft!.draftText); }} className="mt-2 px-3 py-1.5 text-xs font-medium bg-white text-black rounded-lg hover:bg-white">Copy to clipboard</button>
                          </div>
                        )}
                      </div>
                    </div>
            ...
            {pendingProposal && (
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
                ...
                <div className="flex gap-2">
                  ...
                  <button type="button" onClick={handleEditProposal} className="px-3 py-1.5 bg-white text-black text-sm font-medium rounded-lg hover:bg-white">✏️ Edit</button>
                  <button type="button" onClick={handleCancelProposal} className="px-3 py-1.5 bg-white text-black text-sm font-medium rounded-lg hover:bg-white">❌ Cancel</button>
                </div>
              </div>
            )}
          </div>
          {commandsView === 'chat' && (
          <form onSubmit={handleCommandSubmit} className="p-4 border-t border-black bg-white">
...
function ChartRenderer({ chart }: { chart: ChartData }) {
  if (chart.type === 'metric') {
    return (
      <div className="bg-white rounded-xl p-4">
        <p className="text-xs text-black uppercase tracking-wide mb-1">{chart.title}</p>
        <p className="text-3xl font-bold text-black">{chart.data.value}</p>
        {chart.data.subtitle && (
          <p className="text-sm text-black mt-1">{chart.data.subtitle}</p>
        )}
      </div>
    );
  }
  ...
  if (chart.type === 'bar') {
    ...
    return (
      <div className="bg-white rounded-xl p-4">
        <p className="text-xs text-black uppercase tracking-wide mb-3">{chart.title}</p>
        <div className="space-y-2">
          {chart.data.map((item: any, i: number) => (
            <div key={i} className="flex items-center gap-3">
              <span className="text-xs text-black w-20 truncate">{item.label}</span>
              ...
              <span className="text-xs font-medium text-black w-12 text-right">{item.value}</span>
            </div>
          ))}
        </div>
      </div>
          ...
          <thead>
            <tr className="border-b border-black">
              {chart.labels?.map((label, i) => (
                <th key={i} className="text-left py-2 px-2 text-black font-medium">{label}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {chart.data.map((row: any, i: number) => (
              <tr key={i} className="border-b border-black last:border-0">
                {Object.values(row).map((cell: any, j: number) => (
                  <td key={j} className="py-2 px-2 text-black">{cell}</td>
                ))}
              </tr>
            ))}
          </tbody>
  ...
  return (
    <div className="bg-white rounded-xl p-4">
      <p className="text-xs text-black">{chart.title}</p>
