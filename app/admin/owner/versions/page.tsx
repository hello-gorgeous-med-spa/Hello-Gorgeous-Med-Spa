    ...
    <OwnerLayout title="Version History" description="Track all configuration changes with rollback">
      ...
      <div className="space-y-6">
        ...
        <div className="flex gap-2">
          {categories.map(cat => (
            <button key={cat} onClick={() => setFilter(cat)} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${filter === cat ? 'bg-[#FF2D8E] text-white shadow-lg shadow-[#FF2D8E]/30' : 'bg-pink-100 text-pink-700 hover:bg-pink-200 border border-pink-200'}`}>
              {cat === 'all' ? 'All' : cat}
            </button>
          ))}
        </div>
