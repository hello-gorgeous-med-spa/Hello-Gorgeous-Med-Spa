...
export function generateStaticParams() {
  return plainfieldServiceSlugs.map((service) => ({ service }));
}
...
export function generateMetadata({
  ...
  if (!s) {
    return pageMetadata({
      title: "Plainfield, IL Service",
      description: "Service details for Plainfield, IL.",
      path: "/plainfield-il",
    });
  }

  return pageMetadata({
    title: `${s.name} Near Plainfield, IL`,
    description: `${s.name} near Plainfield, IL — ${s.short} Book a consultation at Hello Gorgeous Med Spa.`,
    path: `/plainfield-il/${s.slug}`,
  });
}

function plainfieldFaqs(serviceName: string) {
  return [
    {
      question: `Do you offer ${serviceName} for Plainfield clients?`,
      answer:
        "Yes—Hello Gorgeous Med Spa serves clients from Plainfield. Our clinic is located in Oswego, IL.",
    },
    {
...
export default function PlainfieldServicePage({ params }: { params: { service: string } }) {
  if (!plainfieldServiceSlugs.includes(params.service as (typeof plainfieldServiceSlugs)[number])) {
    notFound();
  }
  ...
  const breadcrumbs = [
    ...
    { name: "Plainfield, IL", url: `${SITE.url}/plainfield-il` },
    { name: s.name, url: `${SITE.url}/plainfield-il/${s.slug}` },
  ];
  ...
  return (
    <>
      ...
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(faqJsonLd(plainfieldFaqs(s.name))) }}
      />
      <script
        type="application/ld+json"
        ...
        <div className="relative z-10">
          <FadeUp>
            ...
            <div className="mt-10 flex flex-col sm:flex-row gap-4">
              ...
              <CTA href="/plainfield-il" variant="outline">
          ...
          <div className="lg:col-span-7">
            <FadeUp>
              ...
              <p className="mt-4 text-black max-w-2xl">
                This page helps Plainfield clients understand next steps. For full details and FAQs,
            ...
            <div className="mt-10 grid gap-4">
              {[
                ...
                {
                  t: "Serving Plainfield",
                  b: "Convenient for Plainfield clients scheduling in Oswego.",
                },
              ].map((x, idx) => (
      ...
      <Section>
        <FadeUp>
          ...
          <p className="mt-4 text-black max-w-2xl">
            Quick answers for local patients searching for {s.name} near Plainfield, IL.
          </p>
        </FadeUp>

        <div className="mt-10 grid gap-4">
          {plainfieldFaqs(s.name).map((f, idx) => (
