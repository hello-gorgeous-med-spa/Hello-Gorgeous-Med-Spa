...
export async function GET(request: NextRequest) {
  try {
    ...
    // SECURITY: Whitelist to prevent open-redirect attacks
    const rawReturnUrl = request.nextUrl.searchParams.get('returnUrl') || '/admin/settings/payments';
    const returnUrl =
      rawReturnUrl.startsWith('/') &&
      !rawReturnUrl.startsWith('//') &&
      !rawReturnUrl.toLowerCase().includes('http') &&
      !rawReturnUrl.includes('\n') &&
      !rawReturnUrl.includes('\r')
        ? rawReturnUrl
    ...
    // SECURITY: Never cache OAuth redirects
