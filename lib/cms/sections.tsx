...
// Renders dynamic sections from JSON content
// ============================================================
...
function HeroSection({ content }: { content: Record<string, unknown> }) {
  ...
  return (
    ...
    >
      ...
      <div className={`relative z-10 max-w-4xl mx-auto text-${alignment}`}>
        <h1 className={`text-4xl md:text-6xl lg:text-7xl font-bold ${text_color} leading-tight`}>
          {headline}
        </h1>
        {subheadline && (
          <p className={`mt-4 text-xl md:text-2xl ${text_color} opacity-80`}>
            {subheadline}
          </p>
        )}
        {description && (
          <p className={`mt-6 text-lg ${text_color} opacity-70 max-w-2xl ${alignment === 'center' ? 'mx-auto' : ''}`}>
            {description}
          </p>
        )}
        <div className={`mt-8 flex gap-4 ${alignment === 'center' ? 'justify-center' : ''} flex-wrap`}>
          {cta_text && (
            <Link
              href={cta_url}
              className="px-8 py-4 bg-pink-500 hover:bg-pink-600 text-white font-semibold rounded-lg transition-all shadow-lg hover:shadow-xl"
            >
              {cta_text}
            </Link>
          )}
...
function TextSection({ content }: { content: Record<string, unknown> }) {
  ...
  return (
    <section className={`py-16 px-6 ${background_color}`}>
      <div className={`${max_width} mx-auto text-${alignment}`}>
        {title && (
          <h2 className={`text-3xl md:text-4xl font-bold ${text_color} mb-6`}>
            {title}
          </h2>
        )}
        {body && (
          <div 
            className={`prose prose-lg ${text_color} max-w-none`}
        ...
        <div className={`grid grid-cols-1 ${gridCols[columns] || 'md:grid-cols-3'} gap-8`}>
          {services.map((service, idx) => (
            <div key={idx} className="bg-gradient-to-br from-pink-50 to-white rounded-2xl border border-pink-100 p-6 hover:shadow-lg transition-all">
              ...
              {show_cta && service.url && (
                <Link href={service.url} className="mt-4 inline-block text-pink-600 hover:text-pink-700 font-medium">
                  {cta_text} →
                </Link>
              )}
            </div>
  ...
  return (
    <section className={`py-12 px-6 ${background_color}`}>
      <div className="max-w-4xl mx-auto text-center">
        {headline && (
          <h2 className={`text-2xl md:text-3xl font-bold ${text_color}`}>
            {headline}
          </h2>
        )}
        {subheadline && (
          <p className={`mt-2 text-lg ${text_color} opacity-90`}>
            {subheadline}
          </p>
        )}
        {code && (
          <p className={`mt-4 inline-block px-4 py-2 bg-white/20 rounded-lg font-mono font-bold ${text_color}`}>
            Use code: {code}
          </p>
        )}
        {cta_text && (
          <div className="mt-6">
            <Link
              href={cta_url}
              className="px-8 py-3 bg-white text-pink-600 font-semibold rounded-lg hover:bg-pink-50 transition-colors"
            >
              {cta_text}
            </Link>
          </div>
          ...
          {providers.map((p, idx) => (
            <div key={idx} className="bg-gradient-to-br from-pink-50 to-white rounded-2xl p-8 border border-pink-100">
              <div className="flex flex-col md:flex-row gap-6">
                {p.image && (
                  <div className="relative w-32 h-32 rounded-full overflow-hidden flex-shrink-0 border-4 border-pink-200">
...
function CTASection({ content }: { content: Record<string, unknown> }) {
  ...
  return (
    <section className={`py-16 px-6 ${background_color}`}>
      <div className="max-w-4xl mx-auto text-center">
        {title && (
          <h2 className={`text-3xl md:text-4xl font-bold ${text_color}`}>{title}</h2>
        )}
        {subtitle && (
          <p className={`mt-4 text-lg ${text_color} opacity-90`}>{subtitle}</p>
        )}
        <div className="mt-8">
          <Link
            href={cta_url}
            className="inline-block px-10 py-4 bg-white text-pink-600 font-bold rounded-lg hover:bg-pink-50 transition-colors shadow-lg"
          >
            {cta_text}
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          ...
          {show_map && (
            <div className="rounded-xl overflow-hidden h-[300px]">
              <iframe
                src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d2961.123456789!2d-88.3515!3d41.6828!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x0%3A0x0!2zNDHCsDQwJzU4LjEiTiA4OMKwMjEnMDUuNCJX!5e0!3m2!1sen!2sus!4v1234567890"
...
function DividerSection({ content }: { content: Record<string, unknown> }) {
  ...
  if (style === 'blank') {
    return <div className={spacingClasses[spacing] || 'py-8'} />;
  }

  return (
    <div className={`${spacingClasses[spacing] || 'py-8'} px-6`}>
      <hr className="max-w-xl mx-auto border-t-2 border-pink-200" />
    </div>
  );
}
    ...
    <section className="py-20 px-6 bg-gradient-to-br from-pink-500 via-rose-500 to-pink-600">
      <div className="max-w-4xl mx-auto text-center">
        ...
        <div className="mt-8">
          <Link
            href={cta_url}
            className="inline-block px-12 py-5 bg-white text-pink-600 font-bold text-lg rounded-xl hover:bg-pink-50 transition-all shadow-xl"
          >
            {cta_text}
          </Link>
        </div>
