...
function fallbackReply(surface: PeppySurface, question: string): string {
  ...
  if (surface === "ops") {
    ...
    if (/formu|paste|sku|pharmacy|order/.test(q)) {
      return "Formulation first. After Ryan approves, open /admin/rx/regen-orders and tap Send to Formulation. That charges the clinic account. BoomRx only if Formulation does not carry the blend — never say BoomRx to the patient.";
    }
    if (/refund|decline|stripe|bluefin|charm/.test(q)) {
      return "Ryan declines → if they already paid, same-day refund in Charm (Billing → that payment → refund). Paste the Charm receipt id in the chart. No in-app refund button. After compounded/shipped, no vial refund — a licensed Illinois clinician reviews whether they continue. Stripe is retired.";
    }
    if (/faster|skip|video|square|consult/.test(q)) {
      return "Preferred door: tryregenrx.com/consult → Square $49 Medical Visit. Talk first. If a licensed Illinois clinician prescribes, credit $49 on the first Charm invoice and note the Square receipt. If he says no, they paid for the visit — no therapy refund. /start still exists; invoice after he reviews.";
    }
    return "Open /ops/playbook for the bible and /ops/clinical for NPA sheets. I run the clinic path: Today → Chart → Ryan yes/no → Send to Formulation → tracking → portal message. Ask me a specific step.";
  }
  ...
  if (/price|cost|how much/.test(q)) {
    return "Ryan's $49 phone consult is on Square (tryregenrx.com/consult) and credits toward the first therapy invoice if he prescribes. Menus start around $100 weight loss, $49 sexual health, $40 hair, $125 skin, $149 hormones, $73 energy & longevity. Shipping is $30 on the clinic invoice. GORGEOUS20 is 20% off the first medication — we apply it on that invoice, not a website payment screen.";
  }
