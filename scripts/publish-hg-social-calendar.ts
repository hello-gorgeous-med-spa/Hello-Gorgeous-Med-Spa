#!/usr/bin/env npx tsx
/**
 * Publish Hello Gorgeous core brand social calendar (Hello Gorgeous Page).
 *
...
import {
  ...
  listHgSocialSlugs,
  type HgSocialPost,
} from "../lib/marketing/hg-social-calendar-july-2026";
...
async function main() {
  ...
  if (!week && !slug && !all) {
    console.error("Usage:");
    console.error("  npx tsx scripts/publish-hg-social-calendar.ts --week=1");
    console.error("  npx tsx scripts/publish-hg-social-calendar.ts --all [--dry-run]");
    console.error("\nSlugs:", listHgSocialSlugs().join(", "));
    process.exit(1);
  }

  const posts = getHgSocialPosts({ week, slug });
  if (!posts.length) {
    console.error("No posts matched.");
    process.exit(1);
  }

  console.log(`Publishing ${posts.length} Hello Gorgeous post(s)${dryRun ? " [DRY RUN]" : ""}`);
