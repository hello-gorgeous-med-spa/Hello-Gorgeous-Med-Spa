#!/usr/bin/env npx tsx
/**
 * Publish Your Brow Journey social calendar posts (Hello Gorgeous Page — default metaBrand).
 *
...
import {
  BROW_SOCIAL_CALENDAR_JULY_2026,
  getBrowSocialPosts,
  listBrowSocialSlugs,
  type BrowSocialPost,
} from "../lib/marketing/brow-social-calendar-july-2026";
...
async function main() {
  ...
  if (!week && !slug && !all) {
    console.error("Usage:");
    console.error("  npx tsx scripts/publish-brow-social-calendar.ts --week=1");
    console.error("  npx tsx scripts/publish-brow-social-calendar.ts --all [--dry-run]");
    console.error("\nSlugs:", listBrowSocialSlugs().join(", "));
    process.exit(1);
  }

  const posts = getBrowSocialPosts({ week, slug });
  if (!posts.length) {
    console.error("No posts matched.");
    process.exit(1);
  }

  console.log(`Publishing ${posts.length} Brow Journey post(s)${dryRun ? " [DRY RUN]" : ""}`);
