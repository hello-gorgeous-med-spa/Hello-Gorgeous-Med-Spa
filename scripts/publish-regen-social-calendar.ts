...
async function main() {
  ...
  if (!week && !slug && !all) {
    console.error("Usage:");
    console.error("  npx tsx scripts/publish-regen-social-calendar.ts --week=1");
    console.error("  npx tsx scripts/publish-regen-social-calendar.ts --all [--dry-run]");
    console.error("\nSlugs:", listRegenSocialSlugs().join(", "));
    console.error("\nCalendar:");
    for (const p of REGEN_SOCIAL_CALENDAR_JULY_2026) {
      console.log(`  W${p.week} ${p.dayLabel.padEnd(8)} ${p.slug}`);
    }
    process.exit(1);
  }

  const posts = getRegenSocialPosts({ week, slug });
  if (!posts.length) {
    console.error("No posts matched.");
    process.exit(1);
  }

  console.log(`Publishing ${posts.length} RE GEN post(s)${dryRun ? " [DRY RUN]" : ""}`);
