#!/usr/bin/env npx tsx
/**
 * Publish Botox & Fillers social calendar posts.
 *
...
import {
  INJECTABLES_SOCIAL_CALENDAR_JULY_2026,
  getInjectablesSocialPosts,
  listInjectablesSocialSlugs,
  type InjectablesSocialPost,
} from "../lib/marketing/injectables-social-calendar-july-2026";
...
async function main() {
  ...
  if (!week && !slug && !all) {
    console.error("Usage:");
    console.error("  npx tsx scripts/publish-injectables-social-calendar.ts --week=1");
    console.error("  npx tsx scripts/publish-injectables-social-calendar.ts --all [--dry-run]");
    console.error("\nSlugs:", listInjectablesSocialSlugs().join(", "));
    process.exit(1);
  }

  const posts = getInjectablesSocialPosts({ week, slug });
  if (!posts.length) {
    console.error("No posts matched.");
    process.exit(1);
  }

  console.log(`Publishing ${posts.length} injectables post(s)${dryRun ? " [DRY RUN]" : ""}`);
