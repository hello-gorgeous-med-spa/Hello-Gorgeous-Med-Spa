# Google Business Profile Optimization Checklist

Use this checklist to optimize your Google Business Profile (GBP) for local SEO and to reinforce Hello Gorgeous's position as the #1 Best Med Spa in Oswego.

## Business Description (750 characters max)

**Recommended description:**

> Hello Gorgeous Med Spa is the #1 Best Med Spa in Oswego, IL — recognized as Best of Oswego for Best Med Spa, Best Skincare Clinic, Best Medical Weight Loss, and Best Facial Treatments. We are the only Oswego-area med spa with a full-authority nurse practitioner on site as owner, plus Quantum RF, Morpheus8 Burst, and Solaria CO2 fractional laser — the latest Class 4 medical laser technology.
>
> We offer Botox ($10/unit), dermal fillers, medical weight loss (Semaglutide & Tirzepatide), hormone therapy (BioTE), peptide therapy, PRP/PRF, Morpheus8, Quantum RF, Solaria CO2 laser, laser hair removal, facials, IV therapy, and more. All under one roof with licensed medical oversight.
>
> Serving Oswego, Naperville, Aurora, Plainfield, Yorkville, and surrounding areas. Free consultations. Financing available (CareCredit, Cherry, Affirm).

## Services to Add/Verify

Ensure these are listed in your GBP Services section:

### Injectables
- Botox
- Dysport
- Jeuveau
- Dermal fillers
- Lip filler

### Medical Weight Loss
- Semaglutide (GLP-1)
- Tirzepatide (GLP-1/GIP)

### Hormone Therapy
- BioTE hormone pellets
- Testosterone therapy

### Advanced Skin & Body
- Morpheus8 (RF microneedling)
- Morpheus8 Burst (deep RF microneedling at 8mm)
- Quantum RF (body contouring)
- Solaria CO2 laser (skin resurfacing)
- Laser hair removal
- Microneedling
- Chemical peels
- HydraFacial

### Regenerative
- PRP/PRF
- IV therapy
- Peptide therapy

### Wellness
- Vitamin injections
- Cryotherapy

## Attributes to Enable

- **Identifies as women-owned** (if applicable)
- **LGBTQ+ friendly** (if applicable)
- **Wheelchair accessible** (if applicable)
- **Appointment required** (or "Appointments recommended")
- **Online appointments**
- **Free consultations**

## Photos to Add/Update

1. **Exterior** — Building/storefront
2. **Interior** — Treatment rooms, reception
3. **Technology** — Morpheus8, Quantum RF, Solaria CO2 devices (if allowed)
4. **Team** — Danielle, Ryan, staff
5. **Before/After** — With patient consent (blur faces if needed)
6. **Services** — Injectables setup, weight loss consultation, etc.

## Google Posts (Weekly)

Use your Campaign Studio or create posts that highlight:
- Best of Oswego rankings
- NP on site / full prescriptive authority
- Quantum RF, Morpheus8 Burst, Solaria CO2
- Special offers (Botox $10/unit, Spring Break weight loss, etc.)
- Before/after results (with consent)
- Educational content (what to look for in a med spa, etc.)

## Review Strategy

1. **Request reviews** — After every successful visit, send follow-up (email/SMS) with link to: https://g.page/r/CYQOWmT_HcwQEBM/review
2. **Respond to all reviews** — Thank positive reviewers; address any concerns professionally
3. **QR code** — Add review link QR code at front desk or in aftercare materials

## NAP Consistency

Ensure Name, Address, Phone are **identical** across:
- Google Business Profile
- Website (hellogorgeousmedspa.com)
- Yelp, Healthgrades, Facebook, etc.

**Canonical NAP:**
- **Name:** Hello Gorgeous Med Spa
- **Address:** 74 W. Washington Street, Oswego, IL 60543
- **Phone:** (630) 636-6193

## Key Search Terms to Target

Your site and GBP should rank for:
- best med spa oswego il
- nurse practitioner med spa oswego
- Quantum RF oswego
- Morpheus8 Oswego
- Solaria CO2 laser oswego
- Botox oswego
- weight loss oswego
- semaglutide oswego

---

*Last updated: March 2025*
