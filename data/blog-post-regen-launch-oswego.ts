import type { BlogPost } from "./blog-types";

export const regenLaunchOswegoPost: BlogPost = {
  slug: "introducing-re-gen-online-rx-oswego-il",
  title:
    "Introducing RE GEN: Online Rx Care From Hello Gorgeous Med Spa in Oswego, IL",
  metaTitle:
    "RE GEN Online Rx | GLP-1, Peptides, Hormones & Hair Rx | Oswego IL",
  metaDescription:
    "RE GEN is Hello Gorgeous Med Spa's NP-directed telehealth Rx arm in Oswego, IL — start a health intake, meet your provider, and pick up or get prescriptions shipped across Illinois after approval.",
  excerpt:
    "RE GEN brings medical weight loss, peptides, hormones, sexual health, and prescription hair & skin care online — still supervised by Ryan Kent, FNP-BC, still Hello Gorgeous. Here's how the consult-first process works and where to start.",
  category: "Hello Gorgeous RX",
  date: "2026-07-02",
  readTime: "8 min",
  lastReviewed: "2026-07-02",
  keywords: [
    "RE GEN Hello Gorgeous",
    "online Rx Oswego IL",
    "telehealth med spa Illinois",
    "compounded GLP-1 Oswego",
    "prescription hair loss Illinois",
    "NP supervised telehealth Naperville",
    "Hello Gorgeous RX",
    "Ryan Kent FNP Oswego",
  ],
  featuredImage: "/images/regen/regen-og-image.jpg",
  structuredDataFaqs: [
    {
      question: "What is RE GEN by Hello Gorgeous?",
      answer:
        "RE GEN is the prescription and telehealth arm of Hello Gorgeous Med Spa in Oswego, Illinois. It offers NP-directed medical weight loss, peptides, hormone therapy, sexual health support, and prescription hair and skin protocols — with online intake, a provider consult, clinic pickup or flat $30 shipping.",
    },
    {
      question: "How does RE GEN ordering work?",
      answer:
        "You browse protocols and complete a health intake — free to submit. A $49 fee reserves your consult with Ryan Kent, FNP-BC, who reviews your history and sets your protocol. You are invoiced for the medication only after he approves it, then you pick it up in Oswego or have it shipped. Nothing prescription is sold without a consult.",
    },
    {
      question: "Who supervises RE GEN prescriptions?",
      answer:
        "Ryan Kent, FNP-BC — a board-certified family nurse practitioner on site at Hello Gorgeous Med Spa in Oswego. RE GEN is not an anonymous national telehealth app; it is local, relationship-based medical care extended online across Illinois.",
    },
    {
      question: "What RE GEN treatments are available online?",
      answer:
        "Medical weight loss (compounded GLP-1 programs), peptide protocols, hormone therapy, sexual wellness prescriptions, prescription dermatology and hair regrowth (including ManeTain and minoxidil), and related wellness Rx — each with category guides and FAQs on hellogorgeousmedspa.com/rx.",
    },
  ],
  content: `# Introducing RE GEN: Online Rx Care From Hello Gorgeous in Oswego, IL

**Hello Gorgeous Med Spa in Oswego, IL** — serving **Naperville, Aurora, Plainfield, Yorkville, Montgomery**, and the **Fox Valley** — has always been a place where real providers know your name. Today we're extending that same standard of care online with **RE GEN** — our prescription and telehealth arm for medical weight loss, peptides, hormones, sexual health, and prescription hair & skin protocols.

**RE GEN** stands for *Renew. Rebalance. Regenerate.* It is not a separate company or a pop-up telehealth mill. It is **Hello Gorgeous RX™**, supervised by **Ryan Kent, FNP-BC**, with the same Oswego team, the same licensed compounding pharmacy partners, and the same refusal to ship anything without clinical approval.

> Patients trust Hello Gorgeous with a **5.0★ Fresha rating** across nearly 2,000 reviews and **4.4★ on Google** — because we combine medical credibility with relationship-based care, not anonymous apps.

## Why we built RE GEN

For years, clients asked us the same question: *"Can I get my peptide / GLP-1 / hormone refill without driving in every time?"* At the same time, national telehealth brands flooded Illinois with fast prescriptions and little follow-through.

We believed there was a better path — **local NP oversight** with **modern online intake**. RE GEN is that path: online convenience without giving up the provider relationship that makes Hello Gorgeous different.

## How RE GEN works (intake → consult → approval → filled)

We designed the flow to be clear and medical-grade:

1. **Browse & start intake** — Read about each protocol at **[hellogorgeousmedspa.com/rx](/rx)** and complete a category-aware health history (allergies, medications, goals, consent). Free to submit.
2. **Reserve your consult** — A **$49** fee holds your visit with the NP. Medication cost is quoted separately.
3. **Meet your provider** — Ryan Kent, FNP-BC reviews your chart and meets you by secure video or in Oswego, then sets your protocol and dose.
4. **Approved → filled** — You're invoiced for the medication only after sign-off. Pick it up in clinic or ship **flat $30** with tracking via email/SMS.

This is intentional. You are never charged for a prescription you haven't been approved for — **clinical approval comes first**.

## Explore by goal — with Q&A on every category

We built dedicated landing pages so you can read honest product info and FAQs **before** you buy — not after you're already confused.

| Goal | Start here |
|------|------------|
| **Weight loss (GLP-1)** | [RE GEN Weight Loss](/rx/weight-loss) — semaglutide & tirzepatide programs |
| **Hair & skin Rx** | [RE GEN Hair & Skin](/rx/hair-skin) — ManeTain, minoxidil, Glow, Miracle, Erase & GHK-Cu |
| **Hormones** | [RE GEN Hormones](/rx/hormones) — TRT, bioidentical HRT, lab-guided dosing |
| **Sexual health** | [RE GEN Sexual Wellness](/rx/sexual-health) — discreet ED & libido support |
| **Peptides** | [Hello Gorgeous RX peptides](/rx/peptides) — BPC-157, NAD+, sermorelin & more |
| **Brochure & pricing** | [RE GEN brochure](/rx/brochure) — shareable program overview |

Each page includes **product pricing**, a **credibility band** (who reviews your order, which pharmacies we use), and a **FAQ accordion** — the questions patients actually ask before committing.

## Hair & skin: why we gave it its own home

Prescription dermatology and hair regrowth are not afterthoughts. Thinning hair and stubborn skin concerns need **real actives at prescription strength** — not another OTC serum that overpromises.

Our **[Hair & Skin hub](/rx/hair-skin)** covers **ManeTain** (minoxidil 5% leave-in spray), oral and injectable minoxidil options, and compounded creams like **Glow** (even tone), **Miracle** (repair), and **Erase** (acne). Every formula is reviewed for your history before it ships.

## Medical weight loss: still local, now online

If GLP-1 therapy is your goal, start at **[RE GEN Weight Loss](/rx/weight-loss)**. We offer compounded **semaglutide** and **tirzepatide** programs with NP titration — the same clinical seriousness as our in-spa **[medical weight loss services](/services/weight-loss)**, extended to your door.

For a deeper dive on tirzepatide specifically, read our founder's letter: **[A Letter About Our Tirzepatide Program](/blog/a-letter-about-our-tirzepatide-program)**.

## Who this is for — and who it is not for

**RE GEN may be a fit if you:**
- Live in **Illinois** and want NP-supervised Rx shipped home
- Value knowing **who** reviews your chart (Ryan Kent, FNP-BC — not a rotating call center)
- Want transparent category pricing before you pay

**RE GEN is not:**
- A guarantee of treatment — eligibility is always a medical decision
- Appropriate without honest health disclosure (pregnancy, nitrates with ED meds, Type 1 diabetes with GLP-1, etc.)
- A replacement for emergency care — call 911 or your PCP for urgent issues

Individual results vary. Compounded medications are prepared by licensed US pharmacies for individual patients when medically appropriate; they are not FDA-approved brand products.

## Ready to start?

- **Browse the storefront:** [hellogorgeousmedspa.com/rx](/rx)
- **Pick a category guide:** [Weight loss](/rx/weight-loss) · [Hair & skin](/rx/hair-skin) · [Hormones](/rx/hormones) · [Sexual health](/rx/sexual-health)
- **Questions?** Call **[630-636-6193](tel:6306366193)** — we're in Oswego seven days a week.

*RE GEN by Hello Gorgeous Med Spa · 74 W Washington Street, Oswego, IL 60543*

---

## Frequently Asked Questions

### What is RE GEN?

RE GEN is Hello Gorgeous Med Spa's online prescription brand — medical weight loss, peptides, hormones, sexual health, and hair/skin Rx — supervised by Ryan Kent, FNP-BC in Oswego, IL.

### Do I have to come to the spa?

Telehealth covers most new protocols. In-person care is always available at our Oswego location when you prefer face-to-face relationship-based care.

### How much is shipping?

Flat **$30** per RE GEN order after NP approval.

### Is RE GEN the same as Hello Gorgeous?

Yes. Same owners, same NP, same standards — RE GEN is simply how we deliver Rx online across Illinois.

### Where can I see pricing before I order?

Visit **[/rx/brochure?tab=pricing](/rx/brochure?tab=pricing)** or the category pages linked above for patient pricing on popular protocols.`,
};
