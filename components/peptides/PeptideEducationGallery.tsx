      ...
      <FadeUp>
        <div className="text-center mb-10">
          ...
          <h2 className="text-3xl md:text-4xl font-black text-black">
            All <span className="text-[#E6007E]">22</span> peptide education sheets
          </h2>
      ...
      <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
        {PEPTIDE_EDUCATION_THUMBNAILS.map((item, idx) => (
          <FadeUp key={item.slug} delayMs={idx * 25}>
            ...
            >
              <PeptidePickerThumbnail
                src={getPeptidePickerThumbnail(item.slug)?.src ?? item.thumbnailWebp}
                alt={item.alt}
              />
