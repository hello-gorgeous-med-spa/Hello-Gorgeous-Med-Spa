...
import { PEPTIDE_LEARN_CITIES, type PeptideLearnPageModel } from "@/lib/peptide-learn-page";
...
export function PeptideLearnPageContent({ page }: { page: PeptideLearnPageModel }) {
    ...
    <div className="min-h-[100dvh] bg-black font-sans text-white">
      <nav className="sticky top-0 z-40 border-b border-white/10 bg-black/82 backdrop-blur-md">
        <div className="mx-auto flex max-w-[1200px] items-center justify-between px-6 py-3.5">
          ...
          <div className="hidden items-center gap-6 text-[15px] lg:flex">
            ...
            <JourneyPinkBtn href={page.intakeHref} className="!px-5 !py-2.5 !text-[15px]">
              Start intake
            </JourneyPinkBtn>
          </div>
        </div>
        {navOpen ? (
          <div className="border-t border-white/10 px-6 py-4 lg:hidden">
            <div className="flex flex-col gap-3">
              ...
              <JourneyPinkBtn href={page.intakeHref} className="mt-2 w-full">
                Start intake
              </JourneyPinkBtn>
            </div>
          </div>
      ...
      <header className={JOURNEY_HERO_BG}>
        ...
        <div className="relative mx-auto grid max-w-[1200px] gap-10 px-6 py-16 lg:grid-cols-2 lg:items-center lg:gap-14 lg:py-24">
          <div>
            <JourneyEyebrow>{page.eyebrow}</JourneyEyebrow>
            <h1 className="mt-4 font-serif text-[44px] font-bold leading-[1.02] text-white lg:text-[66px]">
              {page.h1} <span className="text-[#FF2D8E]">{page.h1Accent}</span>
            </h1>
            <p className="mt-5 max-w-lg text-lg leading-relaxed text-white/80 lg:text-xl">{page.lede}</p>
            <div className="mt-8 flex flex-wrap gap-4">
              <JourneyPinkBtn href={page.intakeHref}>Start intake</JourneyPinkBtn>
              <JourneyGhostBtn href={BOOK}>{PRIMARY_BOOKING_CTA.label}</JourneyGhostBtn>
            </div>
            <div className="mt-6 flex flex-wrap gap-2.5">
              {[
                `From $${page.fromUsd}/mo`,
                `$${page.consultUsd} NP consult`,
                PRESCRIBING_NP.displayName,
                `Pickup or $${page.shippingUsd} IL ship`,
              ].map((chip) => (
      ...
      >
        <div className="mx-auto grid max-w-[1200px] items-start gap-10 lg:grid-cols-[1.15fr_0.85fr] lg:gap-14">
          <div>
            <JourneySectionHead
              eyebrow={page.whatEyebrow}
              title={page.whatTitle}
              titleAccent={page.whatAccent}
              description={page.whatDescription}
            />
      ...
      >
        <div className="mx-auto max-w-[1200px]">
          <JourneySectionHead
            eyebrow="The Science"
            title={page.scienceTitle}
            titleAccent={page.scienceAccent}
            description={page.scienceDescription}
          />
          <div className="mt-12 grid gap-5 md:grid-cols-2">
            {page.science.map((step) => (
            ...
            <div className="mt-12 grid gap-5 lg:grid-cols-3">
              {page.research.map((card) => (
                <JourneyDarkCard key={card.id}>
                  <p className="text-[12px] font-extrabold uppercase tracking-[0.16em] text-[#FF2D8E]">{card.title}</p>
        ...
        >
          <div className="mx-auto max-w-[1200px]">
            <JourneySectionHead
              ...
              titleAccent={page.compare.titleAccent}
              description={page.compare.description}
            />
            <div className="mt-10 overflow-hidden rounded-[20px] border border-white/14">
              <div className="grid grid-cols-[1.1fr_1fr_1fr] bg-[#FF2D8E] px-4 py-3 text-[13px] font-extrabold uppercase tracking-[0.08em] text-black sm:px-6">
                <span> </span>
                <span>{page.compare.leftLabel}</span>
                <span>{page.compare.rightLabel}</span>
              </div>
              {page.compare.rows.map((row, i) => (
                <div
                  key={row.label}
                ...
                >
                  ...
                  <span className="text-white/90">{row.left}</span>
                  <span className="text-white/80">{row.right}</span>
                </div>
              ))}
            </div>
            <p className="mt-4 max-w-3xl text-[14px] leading-relaxed text-white/55">
              {page.compare.note}{" "}
              <Link
                href={page.compare.otherHref}
                className="font-bold text-[#FF2D8E] underline-offset-4 hover:underline"
              >
                {page.compare.otherLabel}
              </Link>
            </p>
          </div>
        </section>
        ...
        <div className="mx-auto grid max-w-[1200px] items-center gap-10 lg:grid-cols-[0.82fr_1.18fr] lg:gap-14">
          ...
          <div>
            ...
            <p className="mt-5 max-w-xl text-lg leading-relaxed text-white/85">
              Every {page.providerNoun} is prescribed and managed by {PRESCRIBING_NP.displayName} — a
              board-certified family nurse practitioner with full Illinois prescriptive authority, on site
              six days a week in Oswego.
            </p>
          ...
          <div className="mt-10 grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
            <JourneyDarkCard>
              <p className="text-[12px] font-extrabold uppercase tracking-[0.16em] text-[#FF2D8E]">
                {page.pricingTableLabel ?? "Published from-price"}
              </p>
              {page.priceRows?.length ? (
                <ul className="mt-5 divide-y divide-white/10">
                  {page.priceRows.map((row) => (
                    <li key={row.label} className="flex items-baseline justify-between gap-4 py-3">
                      ...
                      <span className="font-serif text-[22px] font-bold text-white">${row.priceUsd}</span>
                    </li>
                  ))}
                </ul>
              ) : (
                <p className="mt-5 font-serif text-[36px] font-bold">
                  From ${page.fromUsd}
                  <span className="ml-2 text-lg font-semibold text-white/50">/mo</span>
                </p>
              )}
              <p className="mt-4 text-[13px] leading-relaxed text-white/50">{page.pricingDisclaimer}</p>
            </JourneyDarkCard>
            <div className="flex flex-col gap-6">
              ...
              {page.extraOffer ? (
                <JourneyDarkCard>
                  <p className="font-serif text-[22px] font-bold">{page.extraOffer.title}</p>
                  <p className="mt-3 text-[15px] leading-relaxed text-white/70">{page.extraOffer.body}</p>
                  <Link
                    href={page.extraOffer.href}
        ...
        <div className="mx-auto grid max-w-[1200px] gap-6 lg:grid-cols-2">
          <JourneyDarkCard>
            ...
            <h2 className="mt-3 font-serif text-[32px] font-bold leading-tight">
              {page.forTitle} <span className="text-[#FF2D8E]">is for</span>
            </h2>
          ...
          <JourneyDarkCard>
            ...
            {page.notForNote ? (
              <p className="mt-5 text-[14px] leading-relaxed text-white/55">{page.notForNote}</p>
            ) : null}
          </JourneyDarkCard>
        </div>
        {page.sides.length > 0 ? (
      ...
      <footer className="border-t border-white/10 px-6 py-12">
        <div className="mx-auto flex max-w-[1200px] flex-col gap-10 lg:flex-row lg:justify-between">
          ...
          <p className="max-w-xl text-[13px] leading-relaxed text-white/45">
            This page is educational and is not medical advice, a diagnosis, or a guarantee of results.{" "}
            {page.footerDrugName} is prescription-only. NP-directed by {PRESCRIBING_NP.displayName} under
            Medical Director {MEDICAL_DIRECTOR.displayName}. {page.pricingDisclaimer} Prices shown are
            starting points. Individual plans vary.
          </p>
        </div>
      </footer>
    </div>
