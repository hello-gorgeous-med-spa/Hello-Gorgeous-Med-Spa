      ...
      <div className="relative mx-auto grid max-w-7xl gap-10 px-4 py-14 sm:px-6 lg:grid-cols-2 lg:items-center lg:gap-14 lg:px-8 lg:py-20">
        <div>
          ...
          <p className="mt-5 text-xs font-bold uppercase tracking-[0.2em] text-[#FFB8DC]">
            Rejuva Fresh Hydra Spa Infusion · Oswego
          </p>
          ...
          <div className="mt-8 rounded-3xl border-4 border-black bg-white p-5 text-black shadow-[8px_8px_0_0_rgba(230,0,126,0.35)] sm:p-6">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <h3 className="mt-3 text-xl font-black sm:text-2xl">{special.title}</h3>
                <p className="mt-1 text-sm font-medium text-black/70">
                  O₂ spray + 2 premium add-ons · {special.duration}
                </p>
              </div>
              <div className="text-right">
                <p className="text-3xl font-black text-[#E6007E] sm:text-4xl">{special.price}</p>
                <p className="text-xs font-semibold text-black/55">{special.priceNote}</p>
              </div>
            </div>
            <ul className="mt-4 space-y-1.5 text-sm font-medium text-black/80">
              {special.includes.slice(0, 3).map((line) => (
