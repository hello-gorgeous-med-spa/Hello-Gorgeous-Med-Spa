...
export function ProductCard({ product, onOpen }: ProductCardProps) {
  ...
  return (
    <button
      ...
      className="group flex h-full flex-col rounded-[18px] border border-white/14 bg-[#0a0206] p-5 text-left shadow-[0_4px_24px_rgba(0,0,0,0.35)] transition duration-200 hover:-translate-y-1 hover:border-[#FF2D8E] hover:shadow-[0_20px_40px_rgba(255,45,142,0.18)] motion-reduce:transition-none motion-reduce:hover:translate-y-0"
    >
      ...
      <h3 className="text-[15px] font-bold leading-snug text-white">{product.name}</h3>
      <p className="mt-1 flex-1 text-[12.5px] leading-relaxed text-white/55">
      ...
      <div className="mt-4 flex items-end justify-between gap-2">
        <div>
          ...
          <p className="text-[11px] text-white/45">30-day supply</p>
        </div>
        <span className="text-sm font-semibold text-[#FF2D8E] group-hover:underline">
          Details →
        </span>
      </div>
    </button>
...
export function CatalogCartButton() {
  ...
  return (
    <button
      type="button"
      onClick={toggleCart}
      className="relative inline-flex items-center gap-2 rounded-full bg-[#FF2D8E] px-4 py-2.5 text-sm font-extrabold text-black shadow-[0_8px_24px_rgba(255,45,142,0.28)] transition hover:bg-white"
    >
      ...
      Cart
      {itemCount > 0 && (
        <span className="flex h-5 min-w-5 items-center justify-center rounded-full bg-black px-1 text-xs font-bold text-[#FF2D8E]">
          {itemCount}
        </span>
      )}
    </button>
