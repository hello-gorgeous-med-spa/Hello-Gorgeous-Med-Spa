...
const TYPE_STYLES: Record<NotificationType, { bg: string; icon: string; label: string }> = {
  ...
  completed: { bg: 'bg-gray-500/20 text-gray-700', icon: '✔', label: 'Completed' },
};
...
export function CalendarNavBar({
  ...
  return (
    <header className="flex items-center justify-between w-full px-4 py-3 bg-slate-800 border-b border-slate-700 text-white rounded-t-xl">
      {/* Left: Calendar controls */}
      <div className="flex items-center gap-3 flex-wrap">
        <span className="text-slate-400 font-medium text-sm hidden sm:inline">Calendar</span>
        <button
        ...
        <span className="text-slate-500 text-sm hidden md:inline">{locationName}</span>
        {filtersButton}
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="relative" ref={dropdownRef}>
          <button
            ...
            className="relative p-2.5 hover:bg-slate-700 rounded-lg transition-colors"
          ...
          {showNotifications && (
            <div className="absolute right-0 mt-2 w-[380px] max-h-[420px] overflow-hidden bg-white rounded-xl shadow-xl border border-gray-200 z-[100] flex flex-col">
                      ...
                      return (
                        <li key={n.id}>
                          <Link
                            ...
                            onClick={() => setShowNotifications(false)}
                            className="flex gap-3 px-4 py-3 hover:bg-gray-50 transition-colors text-left"
                          >
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <p className="text-gray-600 text-xs mt-0.5 line-clamp-2">{n.description}</p>
                              <p className="text-gray-400 text-xs mt-1">{formatTimeAgo(n.at)}</p>
                            </div>
                          </Link>
                        </li>
                      );
        ...
        <div className="flex bg-slate-700 rounded-lg p-0.5">
          <button
            ...
            className={`px-3 py-1.5 text-sm font-medium rounded-md transition-all ${
              viewMode === 'day' ? 'bg-slate-600 text-white' : 'text-slate-400 hover:text-white'
            }`}
          >
            Day
          </button>
          <button
            ...
            className={`px-3 py-1.5 text-sm font-medium rounded-md transition-all ${
              viewMode === 'week' ? 'bg-slate-600 text-white' : 'text-slate-400 hover:text-white'
            }`}
          >
            Week
          </button>
