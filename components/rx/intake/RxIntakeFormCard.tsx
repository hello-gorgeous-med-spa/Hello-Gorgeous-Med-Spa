"use client";

import type { ReactNode } from "react";

export function RxIntakeFormCard({
  stepIndex,
  stepCount,
  stepTitle,
  stepLabels,
  children,
}: {
  /** Omit on single-page forms to hide the step counter and progress bar. */
  stepIndex?: number;
  stepCount?: number;
  stepTitle: string;
  /** Optional short labels under progress bar (mobile-friendly) */
  stepLabels?: string[];
  children: ReactNode;
}) {
  const stepped = typeof stepIndex === "number" && typeof stepCount === "number";
  const index = stepIndex ?? 0;
  const count = stepCount ?? 0;

  return (
    <div className="overflow-hidden rounded-3xl border-4 border-black bg-white shadow-[8px_8px_0_0_rgba(230,0,126,0.35)]">
      <div className="border-b border-black/10 bg-gradient-to-r from-[#FFF0F7] to-white px-5 py-5 md:px-8">
        {stepped && (
          <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#E6007E]">
            Step {index + 1} of {count}
          </p>
        )}
        <h2 className="mt-1 text-xl font-black text-black md:text-2xl">{stepTitle}</h2>
        {stepped && (
          <div className="mt-4 flex gap-1.5" role="progressbar" aria-valuenow={index + 1} aria-valuemin={1} aria-valuemax={count}>
            {Array.from({ length: count }).map((_, i) => (
              <div
                key={i}
                className={`h-2 flex-1 rounded-full transition-colors ${
                  i <= index ? "bg-[#E6007E]" : "bg-black/10"
                }`}
              />
            ))}
          </div>
        )}
        {stepped && stepLabels && stepLabels.length === count && (
          <div className="mt-2 hidden gap-1 sm:flex">
            {stepLabels.map((label, i) => (
              <span
                key={label}
                className={`flex-1 truncate text-[10px] font-semibold ${
                  i === index ? "text-[#E6007E]" : "text-black/35"
                }`}
              >
                {label}
              </span>
            ))}
          </div>
        )}
      </div>
      <div className="text-black [color-scheme:light] [&_input:not([type=radio]):not([type=checkbox])]:bg-white [&_input:not([type=radio]):not([type=checkbox])]:text-black [&_textarea]:bg-white [&_textarea]:text-black [&_select]:bg-white [&_select]:text-black [&_input::placeholder]:text-black/40 [&_textarea::placeholder]:text-black/40">
        {children}
      </div>
    </div>
  );
}
